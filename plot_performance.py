...
if __name__ == "__main__":
    ...
    for repeat_interval, pattern_size in combinations:

        log_dir = f"data/{input_rate}_{simulation_length}_{simulation_length_test}/{repeat_interval}_{pattern_size}/logs"
        plot_dir = f"data/{input_rate}_{simulation_length}_{simulation_length_test}/{repeat_interval}_{pattern_size}/plots"
        ...
        test_log_dir_2 = os.path.join(log_dir, "rsync_sc", 'test')
        ...
        rsync_dict_2 = {line_metric: [[] for i in range(n_rep)] for line_metric in line_metrics}
        sc_dict_2    = {line_metric: [[] for i in range(n_rep)] for line_metric in line_metrics}
        ...
        for log_file in os.listdir(test_log_dir_0):
            ...
            with open(os.path.join(test_log_dir_2, log_file), 'r') as f:
                lines_2 = f.readlines()
            ...
            for line_metric in line_metrics:
                ...
                for i, line in enumerate(lines_2):
                    rsync_dict_2[line_metric][i].append(float(re.findall(rsync_pattern, line)[0]))
                    sc_dict_2[line_metric][i].append(float(re.findall(sc_pattern, line)[0]))

        for metric_name, metric_val in {"01": (sc_dict_0, rsync_dict_1), "2": (sc_dict_2, rsync_dict_2)}.items():
            sc_dict, rsync_dict = metric_val
            ...
            leg = ax.legend(title="Prediction method", fontsize=20, title_fontsize=22)
            ...
            for i, l in enumerate(xlabels):
                ...
                if val % 5 != 0:
                    xlabels[i] = ''
                plt.gca().set_xticklabels(xlabels, fontsize=20)
            ...
            for i, l in enumerate(ylabels):
                ...
                plt.gca().set_yticklabels(ylabels, fontsize=20)
            ...
            ax.text(0.5, 0.5, text_sc,
                    horizontalalignment='left',
                    verticalalignment='center', transform=ax.transAxes,
                    fontsize=21, weight=text_sc_weight,
                    color='#eb7341')

            ax.text(0.5, 0.6, text_rsync,
                    horizontalalignment='left',
                    verticalalignment='center', transform=ax.transAxes,
                    fontsize=21, weight=text_rsync_weight,
                    color='#6ccdc0')

            fig_path = os.path.join(test_plot_dir, metric_name)
        ...
        fig, ax = plt.subplots()
        ax.set_ylim(0.55, 1.0)
        ax.plot(rep_list, sc_dict_0["median"], linewidth=2.0, label='spike count 1', color='#eb7341')
        ax.fill_between(rep_list, sc_dict_0["25"], sc_dict_0["75"], alpha=0.2, color='#eb7341', lw=0)

        ax.plot(rep_list, rsync_dict_1["median"], linewidth=2.0, label='Rsync 1', color='#6ccdc0')
        ax.fill_between(rep_list, rsync_dict_1["25"], rsync_dict_1["75"], alpha=0.2, color='#6ccdc0', lw=0)

        ax.plot(rep_list, rsync_dict_2["median"], linewidth=2.0, linestyle='dashed', label='Rsync 2', color='#6ccdc0')
        ax.fill_between(rep_list, rsync_dict_2["25"], rsync_dict_2["75"], alpha=0.2, color='#6ccdc0', lw=0)

        ax.plot(rep_list, sc_dict_2["median"], linewidth=2.0, linestyle='dashed', label='spike count 2', color='#eb7341')
        ax.fill_between(rep_list, sc_dict_2["25"], sc_dict_2["75"], alpha=0.2, color='#eb7341', lw=0)
        ...
        fig_path = os.path.join(test_plot_dir, "012")
        fig.savefig(fig_path + '.png', bbox_inches="tight")
        fig.savefig(fig_path + '.svg', bbox_inches="tight")
        plt.close(fig)
        print('file SAVED', fig_path)
